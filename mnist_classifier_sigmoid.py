...
# model
class MnistClassifierSigmoid(nn.Module):
    def __init__(self):
        super().__init__()
...
def mnist_classifier_sigmoid():
    ...
    # model
    model = MnistClassifierSigmoid().to(device)
    ...
    print('Mnist Classifier Sigmoid')
    train_losses = []
    train_accuracies = []
    validation_losses = []
    ...
    for epoch in epochs:
        ...
        # train
        train_loss, train_accuracy = train(model=model, device=device, loader=train_loader,
                                           optimizer=optimizer, loss=loss, info_per_batch=train_info_per_batch)
        ...
        # validation
        validation_loss, validation_accuracy = test(model=model, loader=validation_loader, device=device, loss=loss,
                                                    info_per_batch=validation_info_per_batch, info_name='Validation')
        ...
        # test
        test_loss, test_accuracy = test(model=model, loader=test_loader, device=device, loss=loss,
                                        info_per_batch=test_info_per_batch, info_name='Test')
        ...
        # epoch state
        state_file_name = 'mnist_classifier_sigmoid_epoch_{epoch_index}.pkl'.format(epoch_index=epoch)
        save_state(model=model, directory=path['state'], file_name=state_file_name)

    # train loss
    save_data(array=train_losses, directory=path['array'],
              file_name='mnist_classifier_sigmoid_train_loss.npy')
    draw_line_graph(x=epochs, y=train_losses,
                    x_label='Epoch', y_label='Loss',
                    title='Mnist Classifier Sigmoid Train Loss',
                    directory=path['graph'],
                    file_name='mnist_classifier_sigmoid_train_loss.png')

    # train accuracy
    save_data(array=train_accuracies, directory=path['array'],
              file_name='mnist_classifier_sigmoid_train_accuracy.npy')
    draw_line_graph(x=epochs, y=train_accuracies,
                    x_label='Epoch', y_label='Accuracy',
                    title='Mnist Classifier Sigmoid Train Accuracy',
                    directory=path['graph'],
                    file_name='mnist_classifier_sigmoid_train_accuracy.png')

    # validation loss
    save_data(array=validation_losses, directory=path['array'],
              file_name='mnist_classifier_sigmoid_validation_loss.npy')
    draw_line_graph(x=epochs, y=validation_losses,
                    x_label='Epoch', y_label='Loss',
                    title='Mnist Classifier Sigmoid Validation Loss',
                    directory=path['graph'],
                    file_name='mnist_classifier_sigmoid_validation_loss.png')

    # validation accuracy
    save_data(array=validation_accuracies, directory=path['array'],
              file_name='mnist_classifier_sigmoid_validation_accuracy.npy')
    draw_line_graph(x=epochs, y=validation_accuracies,
                    x_label='Epoch', y_label='Accuracy',
                    title='Mnist Classifier Sigmoid Validation Accuracy',
                    directory=path['graph'],
                    file_name='mnist_classifier_sigmoid_validation_accuracy.png')

    # test loss
    save_data(array=test_losses, directory=path['array'],
              file_name='mnist_classifier_sigmoid_test_loss.npy')
    draw_line_graph(x=epochs, y=test_losses,
                    x_label='Epoch', y_label='Loss',
                    title='Mnist Classifier Sigmoid Test Loss',
                    directory=path['graph'],
                    file_name='mnist_classifier_sigmoid_test_loss.png')

    # test accuracy
    save_data(array=test_accuracies, directory=path['array'],
              file_name='mnist_classifier_sigmoid_test_accuracy.npy')
    draw_line_graph(x=epochs, y=test_accuracies,
                    x_label='Epoch', y_label='Accuracy',
                    title='Mnist Classifier Sigmoid Test Accuracy',
                    directory=path['graph'],
                    file_name='mnist_classifier_sigmoid_test_accuracy.png')
    ...
    draw_multi_lines_graph(
        ...
        x_label='Epoch', y_label='Loss',
        title='Mnist Classifier Sigmoid Loss',
        directory=path['graph'],
        file_name='mnist_classifier_sigmoid_loss.png')
    ...
    draw_multi_lines_graph(
        ...
        x_label='Epoch', y_label='Accuracy',
        title='Mnist Classifier Sigmoid Accuracy',
        directory=path['graph'],
        file_name='mnist_classifier_sigmoid_accuracy.png')


# main
if __name__ == '__main__':
    mnist_classifier_sigmoid()
